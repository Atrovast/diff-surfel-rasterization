...
traceCUDA(
	...
	const float2* __restrict__ points_xy_image,
	const float* __restrict__ features,
	const float* __restrict__ img_sem, // semantics,
	const int* __restrict__ img_mask,
	const float* __restrict__ transMats,
	const float* __restrict__ depths,
	const float4* __restrict__ normal_opacity,
	float* __restrict__ final_T,
	...
	const float* __restrict__ bg_color,
	float* __restrict__ out_color,
	float* __restrict__ gau_sem,
	int* __restrict__ num_gsem)
{
	// Identify current tile and associated min/max pixel range.
	auto block = cg::this_thread_block();
	uint32_t horizontal_blocks = (W + BLOCK_X - 1) / BLOCK_X;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < CHANNELS; ch++)
				C[ch] += features[collected_id[j] * CHANNELS + ch] * w;
			if (img_mask[pix_id] > 0) {
				for (int ch = 0; ch < SCHANNELS; ch++) {
					atomicAdd(&gau_sem[collected_id[j] * SCHANNELS + ch], img_sem[pix_id * SCHANNELS + ch] * alpha * T);
				}
				atomicAdd(&num_gsem[collected_id[j]], 1);
			}
			T = test_T;
...
void FORWARD::trace(
	...
	int* num_gsem)
{
	traceCUDA<NUM_CHANNELS, TRACE_CHANNELS> << <grid, block >> > (
		ranges,
		point_list,
		W, H,
		focal_x, focal_y,
...
template<int C>
__global__ void preprocessCUDA_p(int P, int D, int M,
	const float* orig_points,
	const glm::vec2* scales,
	const float scale_modifier,
	const glm::vec4* rotations,
	...
	float* __restrict__ out_others)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < CHANNELS; ch++)
				C[ch] += features[collected_id[j] * CHANNELS + ch] * w;
			atomicMaxFloat(&max_weight[collected_id[j]], w);
			T = test_T;
...
void FORWARD::render_p(
	...
	float* out_others)
{
	renderCUDA_p<NUM_CHANNELS> << <grid, block >> > (
		ranges,
		point_list,
		W, H,
		focal_x, focal_y,
...
void FORWARD::preprocess_p(int P, int D, int M,
	const float* means3D,
	const glm::vec2* scales,
	const float scale_modifier,
	const glm::vec4* rotations,
	...
	bool prefiltered)
{
	preprocessCUDA_p<NUM_CHANNELS> << <(P + 255) / 256, 256 >> > (
		P, D, M,
		means3D,
		scales,
		scale_modifier,
